...
namespace
{
class t6: public AlphaBase
{
    public:
    explicit t6(XMLCONFIG::Element *cfg):
        AlphaBase(cfg), 
       ...
       for(int ii = 0; ii < GLOBAL::Instruments.size(); ++ ii)
        {
            if((valid[ di ][ ii ]))
            {
                ...
                for (int i = 0 ; i < 48; ++i)
                {
                    ...
                    {
                        alpha[ii] =  QL_Oputils::corr(factor2,factor1) * (QL_Oputils::mean(factor1) - QL_Oputils::mean(factor3)) / sqrt(QL_Oputils::sum(factor2)) ;
                    }
