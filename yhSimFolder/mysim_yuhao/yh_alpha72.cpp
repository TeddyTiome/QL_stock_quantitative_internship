...
namespace
{
class alpha72: public AlphaBase
{
    public:
    explicit alpha72(XMLCONFIG::Element *cfg):
        AlphaBase(cfg), 
	...
	Num7(cfg->getAttributeIntDefault("para7",2))
    ...
    void generate(int di) override
    {
	    ...
	    vector<float> rank1(GLOBAL::Instruments.size(),0);
       ...
       for(int ii = 0; ii < GLOBAL::Instruments.size(); ++ ii)
        ...
        for(int ii = 0; ii < GLOBAL::Instruments.size(); ++ ii)
        {
            if((valid[ di ][ ii ]))
            {
	    		...
	    		for (int i =0; i<Num2; ++i){
	    			c1[i]=0.5*(high[di-delay-i-a][ii]+low[di-delay-i-a][ii]);
	    			c2[i]=adv40[di-delay-i-a][ii];
	    		}
	    		c3[a] = QL_Oputils::corr(c1,c2);
	    		rank1[ii] += float(c3[a]) *(Num3-a) / ((Num3+1)*Num3/2.0);
	    		}
            }
    	}
        ...
        for(int ii = 0; ii < GLOBAL::Instruments.size(); ++ ii)
        {
            if((valid[ di ][ ii ]))
            {
				...
				vector<float> c9(Num7,nan);
				
				    for (int b=0; b<Num7;++b){
				    	for (int j=0; j<Num6; ++j){
				    		for (int i=0;i<Num4;++i){
				    			c5[i] = vwap[di-delay-i-j-b][ii];
				    		}
						...
						c9[b] = QL_Oputils::corr(c6,c8);
						rank2[ii] += float(c9[b]) *(Num7-b) / ((Num7+1)*Num7/2.0);
					}
	    	}
	    }
