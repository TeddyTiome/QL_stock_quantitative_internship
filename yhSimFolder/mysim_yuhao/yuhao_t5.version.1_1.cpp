       ...
       for(int ii = 0; ii < GLOBAL::Instruments.size(); ++ ii)
        {
            if((valid[ di ][ ii ]))
            {
                ...
                for (int i = 0 ; i < 48; ++i)
                {
                    ...
                    {
                    factor1[i] = abs(vwap(di-delay,i+1,ii) - last_close(di-delay,i+1,ii));
                ...
                for (int j = 0; j<48; ++j)
                {
                    if (factor3[j] > Num1)
                    {
                        ...
                        temp3 += factor3[j] * vwap(di-delay,j+1,ii);
                    }
                    factor4[j] = vwap(di-delay,j+1,ii);
                    factor5[j] = vol_sum(di-delay,j+1,ii);
                    factor6[j] = vwap(di-delay,j+1,ii) * vol_sum(di-delay,j+1,ii);
                }

                
                if (QL_Oputils::sum(factor5) == 0 || temp3 ==0)
                {
                    alpha[ii] = 0;
                }                
                else
                    // float temp4 = QL_Oputils::sum(factor6) / QL_Oputils::sum(factor5);
                {
                    alpha[ii] =   temp1 * QL_Oputils::mean(factor4) / temp3;
