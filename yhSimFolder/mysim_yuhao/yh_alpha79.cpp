...
namespace
{
class alpha79: public AlphaBase
{
    public:
    explicit alpha79(XMLCONFIG::Element *cfg):
        AlphaBase(cfg), 
	...
	Num2(cfg->getAttributeIntDefault("para2",150)),
	Num3(cfg->getAttributeIntDefault("para3",3)),
    Num4(cfg->getAttributeIntDefault("para4",9)),
    Num5(cfg->getAttributeIntDefault("para5",14))
    {

        adv150.resize(GLOBAL::Dates.size());
        for (int di =0; di<GLOBAL::Dates.size(); ++di)
        {
            adv150[di].resize(GLOBAL::Instruments.size(),nan);
        }
       ...
       for(int ii = 0; ii < GLOBAL::Instruments.size(); ++ ii)
        {
            if((valid[ di ][ ii ]))
            {
                ...
                adv150[di-delay][ii] = QL_Oputils::mean(c1);
            }
        }
