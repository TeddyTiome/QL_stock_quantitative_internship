       ...
       for(int ii = 0; ii < GLOBAL::Instruments.size(); ++ ii)
        {
            if((valid[ di ][ ii ]))
            {
                ...
                if (QL_Oputils::mean(factor2)>Num1)
                {
                        if (QL_Oputils::sum(factor4) == 0 || QL_Oputils::sum(factor3) == 0 || QL_Oputils::mean(factor2)==0)
                     {
                         alpha[ii] = 0;
                     }
                    else 
                     {
                        alpha[ii] = - QL_Oputils::sum(factor5)/QL_Oputils::sum(factor4) * (QL_Oputils::sum(factor1) / QL_Oputils::sum(factor3)) / QL_Oputils::mean(factor2);
                     }
                }
                else 
                {
