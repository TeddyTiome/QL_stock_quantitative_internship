    ...
    void generate(int di) override
    {
       ...
       for(int ii = 0; ii < GLOBAL::Instruments.size(); ++ ii)
        ...
        for(int ii = 0; ii < GLOBAL::Instruments.size(); ++ ii)
        {
            if((valid[ di ][ ii ]))
            {
                if (!(GLOBALFUNC::isnan(atr[di-delay][ii]))&&!(GLOBALFUNC::equal(atr[di-delay][ii],0))&&!(GLOBALFUNC::isnan(tr[di-delay][ii]))&&!(GLOBALFUNC::equal(tr[di-delay][ii],0)))
                {
                if (close[di-delay][ii]>(ma[di-delay][ii] + 0.5*atr[di-delay][ii]))
                {
                    alpha[ii] =   -1*(high[di-delay][ii] - ma[di-delay][ii] -high[di-delay-1][ii] + ma[di-delay-1][ii]) * powf(atr[di-delay][ii],atr_ts[di-delay][ii]) * tr[di-delay][ii];
                }
                else if (close[di-delay][ii]<(ma[di-delay][ii] - 0.5*atr[di-delay][ii]))
                {
                    alpha[ii] =  -1*(low[di-delay][ii] - ma[di-delay][ii] - low[di-delay-1][ii] + ma[di-delay-1][ii])  * powf(atr[di-delay][ii],atr_ts[di-delay][ii]) *tr[di-delay][ii];                     
                }
                else {alpha[ii] = 0;}
                }
             else {alpha[ii] = 0;}
